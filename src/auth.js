...
async function doLogin() {
  ...
  // SEMPRE busca no Supabase â€” garante ID UUID real para salvar itens
  const db = getSupa();
  let user = null;
  ...
  if (db) {
    try {
      ...
      if (supa && (supa.pass === pass || supa.password_plain === pass)) {
        user = {
          id: supa.id,           // â† UUID real do Supabase
          name: supa.name,
          desc: supa.descricao || '',
          emoji: supa.emoji || 'ðŸ”',
        ...
        // Atualiza/salva no localStorage com ID real
  ...
  if (user.status === 'pending') { alert('Seu cadastro ainda estÃ¡ em anÃ¡lise. Aguarde a aprovaÃ§Ã£o.'); return; }
  if (user.status === 'inactive') {
    currentUser = user; currentStoreSlug = user.slug;
    SESSION.save(user);
...
function loginDemo() {
  ...
  if (demo) {
    ...
    alert('Loja de teste nÃ£o encontrada. Recarregue a pÃ¡gina.');
  }
}
...
function doRegister() {
  ...
  if (!ok) return;
  // Armazena dados temporÃ¡rios para finalizar no pagamento
