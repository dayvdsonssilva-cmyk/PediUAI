...
function initDemoData() {
  if (!LS.get('demo_init')) {
    const demoUser = {
      ...
      orders: [],
      menuItems: [
        {id:1,emoji:'🍔',photo:null,name:'X-Burguer Especial',desc:'Pão brioche, carne 180g, queijo cheddar, alface, tomate e molho especial.',cat:'Lanches',price:28.90,available:true},
        {id:2,emoji:'🍔',photo:null,name:'X-Bacon Duplo',desc:'Duplo de carne, bacon crocante, queijo prato, picles e molho barbecue.',cat:'Lanches',price:36.90,available:true},
        {id:3,emoji:'🍟',photo:null,name:'Batata Frita Grande',desc:'Porção generosa com sal grosso e molho especial.',cat:'Acompanhamentos',price:14.90,available:true},
        {id:4,emoji:'🥤',photo:null,name:'Refrigerante 350ml',desc:'Coca-Cola, Guaraná ou Fanta.',cat:'Bebidas',price:7.90,available:true},
        {id:5,emoji:'🧃',photo:null,name:'Suco Natural',desc:'Laranja, maracujá ou limão, feito na hora.',cat:'Bebidas',price:12.90,available:true},
        {id:6,emoji:'🧁',photo:null,name:'Sundae de Chocolate',desc:'Sorvete de baunilha com calda quente de chocolate belga.',cat:'Sobremesas',price:11.90,available:true},
      ],
      flashItems: [
        {id:1,type:'image',emoji:'🔥',title:'Promoção: 2x1 no X-Burguer hoje!',url:null},
        {id:2,type:'image',emoji:'🎉',title:'Novo! Sundae de Nutella chegou',url:null},
      ],
      welcome: 'Seja bem-vindo! Escolha seu lanche com carinho 🍔',
    };
    demoUser.orders = [
      {id:'#1003',client:'Ana Silva',phone:'(11) 98765-4321',address:'Rua das Flores, 123',delivery:'delivery',items:[{name:'X-Burguer Especial',qty:2,price:28.90},{name:'Refrigerante 350ml',qty:2,price:7.90}],payment:'Pix',total:73.60,status:'new',time:'há 5 min',ts:Date.now()-300000},
      {id:'#1002',client:'Carlos Souza',phone:'(11) 91234-5678',address:'Av. Central, 456',delivery:'delivery',items:[{name:'X-Bacon Duplo',qty:1,price:36.90},{name:'Batata Frita Grande',qty:1,price:14.90}],payment:'Cartão',total:51.80,status:'preparing',time:'há 18 min',ts:Date.now()-1080000},
      {id:'#1001',client:'Fernanda Lima',phone:'(11) 97654-3210',address:'Rua do Bosque, 78',delivery:'pickup',items:[{name:'X-Burguer Especial',qty:1,price:28.90},{name:'Suco Natural',qty:1,price:12.90},{name:'Sundae de Chocolate',qty:1,price:11.90}],payment:'Dinheiro',total:53.70,status:'done',time:'há 1h',ts:Date.now()-3600000},
    ];
    let users = getUsers();
...
function goTo(id) {
  ...
  currentScreen = id;
  if (id === 's-dash') renderDashboard();
  if (id === 's-store') renderStore();
  if (id === 's-checkout') renderCheckout();
  if (id === 's-ceo') renderCeo();
  window.scrollTo(0,0);
}

// Hash-based routing for store links
function initRouting() {
  // Support /restaurante/slug path (Vercel SPA routing via vercel.json rewrites)
  const path = window.location.pathname;
  const pathMatch = path.match(/\/restaurante\/([\w-]+)/);
  if (pathMatch) {
    const slug = pathMatch[1];
    // Try demo first
    if (slug === 'demo') { openDemo(); return true; }
    const users = getUsers();
    // Match by slug (strip timestamp suffix for matching)
    const user = users.find(u => u.slug === slug && u.status === 'active')
      || users.find(u => u.slug.startsWith(slug.replace(/-[a-z0-9]+$/, '')) && u.status === 'active');
    if (user) {
      currentUser = user;
    ...
    // Slug not found — show landing with message
    showNotif('🔍 Loja não encontrada', 'Verifique o link e tente novamente.');
    return false;
  }

  // Hash routing fallback (#store-slug or #demo)
  const hash = window.location.hash;
  if (hash.startsWith('#store-')) {
    const slug = hash.replace('#store-','');
    const users = getUsers();
    const user = users.find(u => u.slug === slug && u.status === 'active');
    if (user) {
      currentUser = user;
  ...
  if (hash === '#demo' || hash === '#/demo') { openDemo(); return true; }
  return false;
}
...
function openDemo() {
  ...
  if (demo) {
    currentUser = demo;
    currentStoreSlug = 'demo';
    goTo('s-store');
  }
}

function goBackFromStore() {
  if (currentUser && currentUser.id !== 'demo' && currentStoreSlug) {
    goTo('s-login');
  } else {
    goTo('sl');
  }
...
// ─── SECRET CEO CODE (desktop + landing only) ─────────────────────────────
let secretSeq = '';
document.addEventListener('keydown', (e) => {
  const isMobile = window.matchMedia('(hover:none) and (pointer:coarse)').matches;
  if (isMobile) return;
  if (currentScreen !== 'sl') return;
  if (e.key && e.key.length === 1 && /\d/.test(e.key)) {
    secretSeq += e.key;
    if (secretSeq.length > 8) secretSeq = secretSeq.slice(-8);
    if (secretSeq === '35585201') { secretSeq = ''; goTo('s-ceo-login'); }
  } else if (e.key === 'Escape') { secretSeq = ''; }
});
...
function doLogin() {
  ...
  if (user.status === 'inactive') { alert('Sua conta está inativa. Entre em contato com o suporte.'); return; }
  currentUser = user;
  currentStoreSlug = user.slug;
  goTo('s-dash');
}
...
function doRegister() {
  ...
  if (!ok) return;
  // Armazena dados temporários para finalizar no pagamento
...
function selectPlan(plan) {
  ...
  const price = billingCycle === 'annual' ? (plan === 'basico' ? 'R$ 29,90/mês' : 'R$ 79,90/mês') : (plan === 'basico' ? 'R$ 39,90/mês' : 'R$ 99,90/mês');
  info.style.display = 'block';
  document.getElementById('psi-title').textContent = (plan === 'basico' ? 'Plano Básico — ' : 'Plano Ultra — ') + price;
  document.getElementById('psi-desc').textContent = plan === 'basico' ? 'Cardápio digital completo com pedidos ilimitados.' : 'Tudo do Básico + Financeiro, relatórios 90 dias e IA de automação.';
}
function goToMercadoPago() {
  ...
  // Em produção: redireciona para o Mercado Pago via backend
  // window.location.href = '/api/subscribe?plan='+selectedPlan+'&billing='+billingCycle;
  alert('Em produção isto abre o Mercado Pago.\n\nPlano ' + (selectedPlan==='basico'?'Básico':'Ultra') + ' · R$ ' + price.toFixed(2).replace('.',',') + '/mês\n\nUse "Simular pagamento" para testar agora.');
}
function simulatePayment() {
  const reg = window._pendingReg;
  if (!reg || !reg.name) { alert('Dados do cadastro não encontrados. Volte e preencha o formulário.'); goTo('s-register'); return; }
  if (!selectedPlan) { alert('Selecione um plano.'); return; }
  const slug = reg.name.toLowerCase().replace(/\s+/g,'-').replace(/[^a-z0-9-]/g,'') + '-' + Date.now().toString(36);
  const newUser = {
    id: Date.now().toString(),
    name: reg.name, desc: reg.desc||'', emoji: '🍔', color: '#E8410A',
...
function renderDashboard() {
  ...
  const todayOrders = orders.filter(o => o.time === 'agora' || o.time.includes('min'));
...
function switchTab(id) {
  ...
  // find the button
  const tabs = ['t-ov','t-menu','t-flash','t-orders','t-financeiro','t-settings'];
...
function switchTabFin() {
  ...
  if (currentUser.plan !== 'ultra') {
    showNotif('Plano Básico','Este recurso requer o Plano Ultra 🚀');
    return;
  }
...
function renderMenuGrid() {
  ...
  if (!items.length) { grid.innerHTML = '<div class="es" style="grid-column:1/-1"><div class="ei">🍽️</div><p>Nenhum item ainda. Adicione o primeiro!</p></div>'; return; }
  grid.innerHTML = items.map((it,i) => `
    <div class="mc">
      <div class="mc-img">${it.photo?`<img src="${it.photo}" alt="">`:''}
        <span class="ef" style="${it.photo?'opacity:0':''}">${it.emoji}</span>
        <span class="mc-badge ${it.available?'av-y':'av-n'}">${it.available?'Disponível':'Pausado'}</span>
      </div>
      <div class="mc-body">
        ...
        <div class="mc-foot"><div class="mp">R$ ${it.price.toFixed(2).replace('.',',')}</div>
          <div class="macts">
            <button class="ib" onclick="toggleAvail(${i})">${it.available?'👁':'🚫'}</button>
            <button class="ib" onclick="editItem(${i})">✏️</button>
            <button class="ib d" onclick="delItem(${i})">🗑</button>
          </div>
        </div>
      </div>
    </div>`).join('');
}
function toggleAvail(i) { if(!currentUser) return; currentUser.menuItems[i].available=!currentUser.menuItems[i].available; saveCurrentUser(); renderMenuGrid(); }
function delItem(i) { if(!currentUser||!confirm('Remover?')) return; currentUser.menuItems.splice(i,1); saveCurrentUser(); renderMenuGrid(); }
...
function addFlash(input) {
  ...
  const r=new FileReader(); r.onload=e=>{
    ...
    currentUser.flashItems.push({id:Date.now(),type:isVid?'video':'image',emoji:'✨',title:file.name.replace(/\.[^.]+$/,''),url:e.target.result});
    saveCurrentUser(); renderFlashAdmin();
  }; r.readAsDataURL(file);
}
function delFlash(i){if(!currentUser)return;currentUser.flashItems.splice(i,1);saveCurrentUser();renderFlashAdmin();}

// ─── ORDERS ──────────────────────────────────────────────────────────────
function sLabel(s){return s==='new'?'Novo':s==='preparing'?'Preparando':s==='ready'?'Pronto':'Entregue';}
function renderOrdersList() {
  ...
  list.innerHTML=orders.map((o,i)=>`<div class="oc">
    ...
    <div class="oacts">
      ...
      ${o.status==='new'?`<button class="btn" style="background:#EEF2FF;color:#3730A3;border:none;font-size:.7rem;padding:.35rem .7rem;font-weight:700" onclick="markPreparing(${i})">🍳 Preparando</button>`:''}
...
function markPreparing(i){if(!currentUser)return;currentUser.orders[i].status='preparing';saveCurrentUser();renderOrdersList();renderOverviewOrders();showCSNotif('preparing');}
function markReady(i){if(!currentUser)return;currentUser.orders[i].status='ready';saveCurrentUser();renderOrdersList();renderOverviewOrders();showCSNotif('ready');}
...
function buildReceipt(o) {
  ...
  return `<div style="background:var(--white);border-radius:16px;padding:1.2rem;border:1px solid var(--border)">
    ...
    <div style="font-size:.82rem;padding:.3rem 0;display:flex;justify-content:space-between;gap:.8rem"><span style="color:var(--ink3);font-weight:600">Entrega</span><span style="text-align:right">${o.delivery==='pickup'?'🏃 Retirada':'🛵 '+o.address}</span></div>
    ...
    <div style="text-align:center;margin-top:.85rem;padding-top:.85rem;border-top:1px dashed var(--border);font-size:.72rem;color:var(--ink3)">Pedido ${o.id} · ${new Date().toLocaleDateString('pt-BR')}<br>Obrigado pela preferência! 💛</div>
  </div>`;
}
...
function renderFinanceiro(period) {
  const lbls={hoje:'Hoje',semana:'Esta semana',mes:'Este mês','90':'90 dias'};
  ...
  const all=[...orders];
  for(let d=1;d<mul;d++) all.push(...orders.map(o=>({...o,id:o.id+'d'+d,time:`há ${d}d`})));
  ...
  if(sf) sf.innerHTML=`<div class="sc"><div class="sc-lbl">Pedidos</div><div class="sc-val o">${count}</div></div><div class="sc"><div class="sc-lbl">Faturamento</div><div class="sc-val g">R$ ${total.toFixed(2).replace('.',',')}</div></div><div class="sc"><div class="sc-lbl">Ticket médio</div><div class="sc-val">R$ ${avg.toFixed(2).replace('.',',')}</div></div><div class="sc"><div class="sc-lbl">Mais usado</div><div class="sc-val" style="font-size:.95rem">Pix</div></div>`;
...
function saveSettings() {
  if(!currentUser) return;
  currentUser.name=document.getElementById('s-name').value;
...
function selColor(c,cd,el){
  ...
  if(currentUser){currentUser.color=c;saveCurrentUser();}
}
const storeEmojis=['🍔','🍕','🌮','🍜','🍣','🍗','🥩','🧆','🥗','🍰','🧁','🍦','🍩','🥐','☕','🍺','🥘','🍱','🎂','🏪','⭐','🌶'];
function updateStoreEmojiPicker(){
  const ep=document.getElementById('store-ep'); if(!ep) return;
  ep.innerHTML=storeEmojis.map(e=>`<button class="epb ${e===(currentUser?.emoji||'🍔')?'sel':''}" onclick="selStoreEmoji('${e}',this)">${e}</button>`).join('');
  if(currentUser){document.getElementById('s-name').value=currentUser.name||'';document.getElementById('s-desc').value=currentUser.desc||'';document.getElementById('s-wel').value=currentUser.welcome||'';document.getElementById('s-wp').value=currentUser.whatsapp||'';}
}
function selStoreEmoji(e,btn){document.querySelectorAll('#store-ep .epb').forEach(b=>b.classList.remove('sel'));btn.classList.add('sel');if(currentUser){currentUser.emoji=e;saveCurrentUser();}}
...
function renderFlashClient(){
  ...
  document.getElementById('flash-client-row').innerHTML=items.map((f,i)=>`<div class="fc" onclick="openFlash(${i})">
    ${f.url?(f.type==='video'?`<video src="${f.url}" class="fc-th" muted playsinline></video>`:`<img src="${f.url}" class="fc-th">`): `<div class="fc-ph">${f.emoji}</div>`}
    <div class="fc-pl">▶</div><div class="fc-ov"><div class="fc-tt">${f.title}</div></div>
  </div>`).join('');
}
...
function renderFlashSlide(){
  ...
  slides.innerHTML=f.url?(f.type==='video'?`<video src="${f.url}" autoplay muted playsinline loop style="width:100%;height:100%;object-fit:cover;position:absolute;inset:0"></video>`:`<img src="${f.url}" style="width:100%;height:100%;object-fit:cover;position:absolute;inset:0">`): `<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:8rem">${f.emoji}</div>`;
  document.getElementById('fs-title').textContent=f.title;
...
function renderClientMenu(cat){
  ...
  if(!filtered.length){list.innerHTML='<div class="es"><div class="ei">🍽️</div><p>Nenhum item nesta categoria</p></div>';return;}
  list.innerHTML=filtered.map(it=>`<div class="ci">
    <div class="ci-in">
      <div class="ci-med">${it.photo?`<img src="${it.photo}" alt="">`:''}<span class="ef" style="${it.photo?'opacity:0':''}">${it.emoji}</span></div>
      <div class="ci-body">
        <div class="ci-name">${it.name}</div>
        <div class="ci-desc">${it.desc}</div>
        <div class="ci-foot"><div class="ci-price">R$ ${it.price.toFixed(2).replace('.',',')}</div>
          <div class="ci-acts"><button class="smb" onclick="openProductModal(${it.id})">Ver mais</button><button class="ab" onclick="addToCart(${it.id},this)">+</button></div>
        </div>
      </div>
    </div>
  </div>`).join('');
...
function renderCheckout(){
  ...
  list.innerHTML=cart.map((it,i)=>`<div class="cir"><div class="ce">${it.emoji}</div><div class="cn">${it.name}</div><div class="qc"><button class="qb" onclick="changeQty(${i},-1)">−</button><span class="qn">${it.qty}</span><button class="qb" onclick="changeQty(${i},1)">+</button></div><div class="cprice">R$ ${(it.qty*it.price).toFixed(2).replace('.',',')}</div></div>`).join('');
...
function placeOrder(){
  ...
  if(deliveryType==='delivery'&&!document.getElementById('cl-addr').value.trim()){alert('Preencha o endereço de entrega');return;}
...
function spawnParticles(){
  const cont=document.getElementById('conf-particles'); cont.innerHTML='';
  const emojis=['🎉','🍔','⭐','✨','🎊','💛'];
  for(let i=0;i<12;i++){
    const p=document.createElement('div'); p.className='particle';
    p.textContent=emojis[i%emojis.length];
    p.style.cssText=`left:${Math.random()*100}%;top:0;animation-duration:${2+Math.random()*3}s;animation-delay:${Math.random()*2}s;font-size:${0.8+Math.random()*0.8}rem;`;
    cont.appendChild(p);
  }
}
...
// ─── MINI GAME ────────────────────────────────────────────────────────────
function startGame(){
  gameActive=true; gameScore=0; bx=100; by=80;
  // Velocidade inicial bem baixa — muito mais fácil
  const speed=1.8+Math.random()*0.8;
  bvx=speed*(Math.random()>.5?1:-1); bvy=speed*(Math.random()>.5?1:-1);
  ...
  b.style.fontSize='2.6rem'; // burger maior, mais fácil de tocar
  moveBurger();
  let t=30; // 30 segundos em vez de 20
...
function moveBurger(){
  ...
  const w=area.clientWidth-56, h=area.clientHeight-56;
  bx+=bvx; by+=bvy;
  if(bx<=0||bx>=w) bvx*=-1;
  if(by<=0||by>=h) bvy*=-1;
  const b=document.getElementById('game-burger');
  if(b){b.style.left=Math.max(0,Math.min(w,bx))+'px';b.style.top=Math.max(0,Math.min(h,by))+'px';}
...
function clickBurger(){
  ...
  // Aceleração muito leve — não fica impossível
  const spd=0.08;
  bvx+=bvx>0?spd:-spd; bvy+=bvy>0?spd:-spd;
  // Trava a velocidade máxima em 3.5
  const maxSpd=3.5;
  if(Math.abs(bvx)>maxSpd) bvx=maxSpd*(bvx>0?1:-1);
  if(Math.abs(bvy)>maxSpd) bvy=maxSpd*(bvy>0?1:-1);
  b.style.transform='scale(1.4)'; setTimeout(()=>{if(b) b.style.transform='';},120);
}
function endGame(){
...
function showCSNotif(status){
  ...
  if(status==='preparing'){
    ...
    bar.innerHTML=`<span class="spinning" style="font-size:1.3rem">🍳</span><div><div style="font-family:'Poppins',sans-serif;font-weight:700;font-size:.86rem;color:#3730A3">Preparando seu pedido!</div><div style="font-size:.72rem;color:var(--ink3);margin-top:.1rem">O chef já está na cozinha 👨‍🍳</div></div>`;
  } else {
    bar.className='csb vis csb-ready';
    bar.innerHTML=`<span style="font-size:1.3rem">✅</span><div><div style="font-family:'Poppins',sans-serif;font-weight:700;font-size:.86rem;color:var(--success)">Pedido pronto!</div><div style="font-size:.72rem;color:var(--ink3);margin-top:.1rem">${status==='pickup'?'Pode vir buscar 🏃':'A caminho! 🛵'}</div></div>`;
  }
  showNotif(status==='preparing'?'🍳 Preparando!':'✅ Pronto!',status==='preparing'?'O chef está na cozinha.':'Seu pedido está a caminho!');
}
...
function doCeoLogin(){
  ...
  // Credenciais CEO — acesso exclusivo do proprietário da plataforma
...
function renderCeo(){
  ...
  const mrr=users.filter(u=>u.status==='active').reduce((s,u)=>s+(u.plan==='ultra'?99.90:39.90),0);
  ...
  if(rb) rb.innerHTML=users.slice(0,5).map((u,i)=>`<tr style="cursor:pointer" onclick="viewCeoStore(${i})">
    ...
    <td><span style="background:${u.plan==='ultra'?'rgba(124,58,237,.2)':'rgba(232,65,10,.15)'};color:${u.plan==='ultra'?'#a78bfa':'var(--brand)'};padding:.1rem .4rem;border-radius:4px;font-size:.65rem;font-weight:700">${u.plan==='ultra'?'Ultra':'Básico'}</span></td>
    <td>${u.dueDate||'—'}</td>
  ...
  if(ab) ab.innerHTML=users.map((u,i)=>`<tr>
    ...
    <td>
      <span id="pw-mask-${i}" style="letter-spacing:.08em;font-size:.72rem;color:rgba(255,255,255,.4)">••••••</span>
      <span id="pw-show-${i}" style="display:none;font-size:.72rem;color:#fbbf24;font-weight:700">${u.pass||'—'}</span>
      <button onclick="togglePw(${i})" style="background:none;border:none;cursor:pointer;font-size:.75rem;margin-left:.3rem;color:rgba(255,255,255,.35);padding:0" title="Mostrar/ocultar senha">👁</button>
    </td>
    <td><span style="background:${u.plan==='ultra'?'rgba(124,58,237,.2)':'rgba(232,65,10,.15)'};color:${u.plan==='ultra'?'#a78bfa':'var(--brand)'};padding:.1rem .4rem;border-radius:4px;font-size:.65rem;font-weight:700">${u.plan==='ultra'?'Ultra':'Básico'}</span></td>
    <td style="font-size:.7rem;color:rgba(255,255,255,.5)">${u.dueDate||'—'}</td>
    <td>${u.status==='active'?'<span class="b-act">Ativo</span>':u.status==='pending'?'<span class="b-pend">Pendente</span>':'<span class="b-inact">Inativo</span>'}</td>
    <td style="white-space:nowrap"><button onclick="ceoToggleStoreByIdx(${i})" style="background:${u.status==='active'?'rgba(239,68,68,.1)':'rgba(74,222,128,.1)'};color:${u.status==='active'?'#f87171':'#4ade80'};border:1px solid ${u.status==='active'?'rgba(239,68,68,.2)':'rgba(74,222,128,.2)'};border-radius:6px;padding:.18rem .5rem;font-size:.68rem;font-weight:700;cursor:pointer;font-family:'Poppins',sans-serif">${u.status==='active'?'Desativar':'Ativar'} <span class="ai-badge">🤖</span></button></td>
  </tr>`).join('');

  // Solicitations
  const sb=document.getElementById('ceo-solic-body');
  if(sb) sb.innerHTML=solics.map((s,i)=>`<tr>
    <td style="color:#fff;font-weight:600">${s.name}</td><td>${s.doc||'—'}</td><td>${s.email}</td>
    <td><span style="background:${s.plan==='ultra'?'rgba(124,58,237,.2)':'rgba(232,65,10,.15)'};color:${s.plan==='ultra'?'#a78bfa':'var(--brand)'};padding:.1rem .4rem;border-radius:4px;font-size:.65rem;font-weight:700">${s.plan==='ultra'?'Ultra':'Básico'}</span></td>
    <td><span class="b-pend">MP Pendente</span></td>
    <td>${s.submittedAt||'—'}</td>
    <td style="display:flex;gap:.35rem">
      <button onclick="approveSolic(${i})" style="background:rgba(74,222,128,.13);color:#4ade80;border:1px solid rgba(74,222,128,.25);border-radius:6px;padding:.18rem .5rem;font-size:.68rem;font-weight:700;cursor:pointer;font-family:'Poppins',sans-serif">✅ Aprovar</button>
      <button onclick="rejectSolic(${i})" style="background:rgba(239,68,68,.1);color:#f87171;border:1px solid rgba(239,68,68,.2);border-radius:6px;padding:.18rem .5rem;font-size:.68rem;font-weight:700;cursor:pointer;font-family:'Poppins',sans-serif">❌ Reprovar</button>
    </td>
  </tr>`).join('');

  // Financeiro CEO
  const ch=document.getElementById('ceo-hist');
  const months=['Mar 2025','Fev 2025','Jan 2025','Dez 2024'];
  if(ch) ch.innerHTML=months.map((m,i)=>`<tr><td>${m}</td><td>${Math.max(0,active-i)}</td><td>${Math.max(0,users.filter(u=>u.plan==='ultra').length-i)}</td><td style="color:#4ade80;font-weight:700">R$ ${(mrr*(1-i*0.05)).toFixed(2).replace('.',',')}</td></tr>`).join('');
}
...
function viewCeoStore(i){
  ...
  document.getElementById('csd-items').innerHTML=items.length?items.map((it,j)=>`<div style="display:flex;align-items:center;gap:.6rem;padding:.5rem;background:rgba(255,255,255,.04);border-radius:var(--rs)">
    ...
    <div style="flex:1"><div style="font-size:.8rem;font-weight:600;color:#fff">${it.name}</div><div style="font-size:.7rem;color:rgba(255,255,255,.4)">R$ ${it.price.toFixed(2).replace('.',',')} · ${it.cat}</div></div>
  ...
  </div>`).join(''):'<div style="font-size:.78rem;color:rgba(255,255,255,.35);padding:.5rem">Nenhum item no cardápio ainda.</div>';
...
function ceoToggleStoreByIdx(i){
  const users=getUsers(); const u=users[i]; if(!u) return;
  u.status=u.status==='active'?'inactive':'active';
  // AI auto-action message
  const action=u.status==='active'?'ativada':'desativada';
  setUsers(users);
  showNotif('🤖 IA PEDIWAY',`Loja "${u.name}" foi ${action} automaticamente.`);
  renderCeo();
...
function approveSolic(i){
  const solics=getSolicitations(); const s=solics.splice(i,1)[0];
  const users=getUsers();
  s.status='active'; s.id=s.id||Date.now().toString();
  ...
  showNotif('✅ Aprovado!',s.name+' agora está ativo.');
  renderCeo();
}
...
function rejectSolic(i){
  ...
  showNotif('❌ Reprovado','Solicitação removida.'); renderCeo();
}
...
(function() {
  ...
  scene.addEventListener('mousemove', (e) => {
    ...
    applyTilt(-dy * 12, dx * 16 - 8);
  });
  scene.addEventListener('mouseleave', () => {
    wrap.style.animation = 'floatPhone 4s ease-in-out infinite';
...
function confirmAndSendOrder() {
  ...
  orderCounter++;
  const order={
    id:'#'+orderCounter,client:name,phone,address:addr,delivery:deliveryType,
...
function buildConfirmData() {
  ...
  const items=cart.map(c=>`${c.qty}x ${c.name} — R$ ${(c.qty*c.price).toFixed(2).replace('.',',')}`);
  ...
  if(cdDel) cdDel.innerHTML=`<strong>Nome:</strong> ${name}<br><strong>Telefone:</strong> ${phone}<br><strong>Tipo:</strong> ${deliveryType==='delivery'?'🛵 Entrega — '+addr:'🏃 Retirada no local'}`;
